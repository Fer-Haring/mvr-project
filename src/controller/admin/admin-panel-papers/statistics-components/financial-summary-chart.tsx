...
const FinancialBalanceChart: React.FunctionComponent = () => {
  ...
  const [options] = useState<AgCartesianChartOptions>({
    title: {
      text: formatMessage({ id: 'FINANCE.ARS.TITLE' }), // Título del gráfico
    },
    data: chartData,
    series: [
      {
        ...
        tooltip: {
          renderer: (params): AgTooltipRendererResult => {
            ...
            return {
              content: `${formatMessage({ id: 'FINANCE.MONTH' })}: ${month}<br>${formatMessage({ id: 'FINANCE.HABERES' })}: $${haberes.toLocaleString()} ARS`,
            };
          },
        },
      },
      {
        ...
        tooltip: {
          renderer: (params): AgTooltipRendererResult => {
            ...
            return {
              content: `${formatMessage({ id: 'FINANCE.MONTH' })}: ${month}<br>${formatMessage({ id: 'FINANCE.DEBERES' })}: $${deberes.toLocaleString()} ARS`,
            };
          },
        },
      },
      {
        ...
        tooltip: {
          renderer: (params): AgTooltipRendererResult => {
            ...
            return {
              content: `${formatMessage({ id: 'FINANCE.MONTH' })}: ${month}<br>${formatMessage({ id: 'FINANCE.GANANCIAS' })}: $${ganancias.toLocaleString()} ARS`,
            };
          },
        },
      },
